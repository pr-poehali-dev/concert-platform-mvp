...
// ── Блок настройки email-рассылки ─────────────────────────────────────────
function EmailScheduleBlock({ integration, onUpdated }: { integration: Integration; onUpdated: () => void }) {
...
// ── Панель отчётов ────────────────────────────────────────────────────────
function ReportPanel({ integration, events, onUpdated }: { integration: Integration; events: BarEvent[]; onUpdated: () => void }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Email-расписание */}
      <EmailScheduleBlock integration={integration} onUpdated={onUpdated} />
    </div>
  );
}
...
export default function DashboardBarTab() {
  ...
  return (
    <div className="animate-fade-in max-w-4xl space-y-5">
      ...
      {/* Панель отчётов */}
      {activeInt && !showForm && (
        <ReportPanel integration={activeInt} events={events} onUpdated={loadIntegrations} />
      )}
