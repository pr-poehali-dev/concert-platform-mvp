...
export default function RequisitesSection({ reqForm, reqSaving, reqSaved, onReqFormChange, onSave }: Props) {
  ...
  return (
    <div className="glass rounded-2xl p-6 space-y-4">
      ...
      <div>
        <label className="text-xs text-white/65 uppercase tracking-wider mb-2 block">Организационная форма</label>
        <div className="grid grid-cols-2 gap-2">
          {(Object.entries(COMPANY_LABELS) as [CompanyType, string][]).map(([val, label]) => (
            <button key={val} type="button" onClick={() => set({ companyType: val })}
              className={`py-2.5 px-3 rounded-xl border text-sm transition-all text-left ${reqForm.companyType === val ? "bg-neon-cyan/15 border-neon-cyan/50 text-neon-cyan" : "glass border-white/10 text-white/70 hover:border-white/25 hover:text-white"}`}>
              {label}
            </button>
          ))}
        </div>
      ...
      {reqForm.companyType !== "individual" && (
        <>
          <div>
            <label className="text-xs text-white/65 uppercase tracking-wider mb-1.5 block">Полное наименование</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs text-white/65 uppercase tracking-wider mb-1.5 block">ИНН</label>
              <input value={reqForm.inn} onChange={e => set({ inn: e.target.value.replace(/\D/g, "") })}
                placeholder="10 или 12 цифр" maxLength={12}
                className="w-full glass rounded-xl px-4 py-3 text-white placeholder:text-white/25 outline-none border border-white/10 focus:border-neon-cyan/50 text-sm" />
            </div>
            <div>
              <label className="text-xs text-white/65 uppercase tracking-wider mb-1.5 block">КПП</label>
          ...
          <div>
            <label className="text-xs text-white/65 uppercase tracking-wider mb-1.5 block">ОГРН / ОГРНИП</label>
            <input value={reqForm.ogrn} onChange={e => set({ ogrn: e.target.value.replace(/\D/g, "") })}
              placeholder="13–15 цифр" maxLength={15}
              className="w-full glass rounded-xl px-4 py-3 text-white placeholder:text-white/25 outline-none border border-white/10 focus:border-neon-cyan/50 text-sm" />
          </div>
          <div>
            <label className="text-xs text-white/65 uppercase tracking-wider mb-1.5 block">Юридический адрес</label>
            <input value={reqForm.legalAddress} onChange={e => set({ legalAddress: e.target.value })}
              placeholder="г. Москва, ул. Ленина, д. 1"
              className="w-full glass rounded-xl px-4 py-3 text-white placeholder:text-white/25 outline-none border border-white/10 focus:border-neon-cyan/50 text-sm" />
          </div>
          <div>
            <label className="text-xs text-white/65 uppercase tracking-wider mb-1.5 block">Фактический адрес</label>
          ...
          <p className="text-xs text-white/65 uppercase tracking-wider">Банковские реквизиты</p>
          <div>
            <label className="text-xs text-white/65 uppercase tracking-wider mb-1.5 block">Банк</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs text-white/65 uppercase tracking-wider mb-1.5 block">Расчётный счёт</label>
              <input value={reqForm.bankAccount} onChange={e => set({ bankAccount: e.target.value.replace(/\D/g, "") })}
                placeholder="20 цифр" maxLength={20}
                className="w-full glass rounded-xl px-4 py-3 text-white placeholder:text-white/25 outline-none border border-white/10 focus:border-neon-cyan/50 text-sm" />
            </div>
            <div>
              <label className="text-xs text-white/65 uppercase tracking-wider mb-1.5 block">БИК</label>
      ...
      <div>
        <label className="text-xs text-white/65 uppercase tracking-wider mb-1.5 block">Телефон</label>
