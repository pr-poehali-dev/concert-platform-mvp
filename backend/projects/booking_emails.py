...
def confirm_email(org_name: str, venue_name: str, date_str: str, rental_amount, venue_conditions: str) -> str:
    rent_str = ""
    if rental_amount:
...
def reject_email(org_name: str, venue_name: str, edate: str, venue_conditions: str) -> str:
    reason = ""
    if venue_conditions:
        reason = f"<p style='color:rgba(255,255,255,0.5);font-size:14px;margin:16px 0 0'>Причина: {venue_conditions}</p>"
    return f"""<!DOCTYPE html><html><head><meta charset="utf-8"></head>
<body style="margin:0;padding:0;background:#0d0d1a;font-family:Arial,sans-serif;">
<table width="100%" cellpadding="0" cellspacing="0" style="background:#0d0d1a;padding:40px 20px;">
<tr><td align="center">
