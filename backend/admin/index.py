...
GET  ?action=stats                     — общая статистика
GET  ?action=users&page=1&search=X     — список пользователей
GET  ?action=venues&page=1&search=X    — список площадок
POST ?action=verify_user&id=X          — верифицировать пользователя
POST ?action=verify_venue&id=X         — верифицировать площадку
POST ?action=toggle_admin&id=X         — выдать/снять права администратора
POST ?action=delete_user&id=X          — удалить пользователя (мягкое удаление — banned)
"""
import json
import os
import psycopg2
...
def handler(event: dict, context) -> dict:
    ...
    if method == "GET" and action == "stats":
        ...
        cur.execute(f"SELECT COUNT(*) FROM {SCHEMA}.notifications")
        total_notifs = cur.fetchone()[0]

        # Регистрации за последние 7 дней
        cur.execute(
            f"SELECT COUNT(*) FROM {SCHEMA}.users WHERE created_at >= NOW() - INTERVAL '7 days'"
        )
        new_users_week = cur.fetchone()[0]

        # Последние 5 регистраций
        cur.execute(
            f"SELECT id, name, email, role, city, verified, created_at FROM {SCHEMA}.users ORDER BY created_at DESC LIMIT 5"
        )
        recent = [
            {"id": str(r[0]), "name": r[1], "email": r[2], "role": r[3],
             "city": r[4], "verified": r[5], "createdAt": str(r[6])}
        ...
        return ok({
            ...
            "venueOwners": venue_owners,
            "verifiedUsers": verified_users,
            "totalVenues": total_venues,
            "verifiedVenues": verified_venues,
    ...
    if method == "GET" and action == "users":
        ...
        cur.execute(
            f"""SELECT id, name, email, role, city, verified, is_admin, avatar, avatar_color, created_at
        ...
        # Кол-во площадок у каждого venue-пользователя
